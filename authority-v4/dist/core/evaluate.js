...
function requiredRoleForAmount(lane, amountCents, policy) {
    const t = policy.thresholds[lane];
    if (!t)
        return policy.denyByDefault ? "team_lead" : null;
    if (amountCents <= t.autoExecuteUpTo)
        return null;
    if (amountCents > t.requireDirectorAbove)
        return "director";
    if (amountCents > t.requireManagerAbove)
        return "manager";
    return "team_lead";
}
export function evaluate(commitment, policy) {
